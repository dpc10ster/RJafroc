...
#   if (!file.exists(fn)) {
#     warning(paste0("File not found - generating new ",fn))
#     ret <- PlotRsmOpChr(mu, lambda, nu, OpChType = "wAFROC", 
#                                            lesDistr = lesDistr)
#     saveRDS(ret, file = fn)
#   }
#   
#   ret <- readRDS(fn)
#   t <- PlotRsmOpChr(mu, lambda, nu, OpChType = "wAFROC", lesDistr = lesDistr)
#   # expect_is(t$wAFROCPlot, "ggplot")
# 
# })
# 
...
#     warning(paste0("File not found - generating new ",fn))
#     
#     ret <- PlotRsmOpChr(mu = mu, lambda = lambda, nu = nu, zeta1 = zeta1, OpChType = "wAFROC", lesDistr = lesDistr, legendPosition = "bottom", nlfRange = c(0, 1), llfRange = c(0, 1))
#     saveRDS(ret, file = fn)
#   }
#   
#   ret <- readRDS(fn)
# # expect_equal(PlotRsmOpChr(mu = mu, lambda = lambda, nu = nu,  zeta1 = zeta1, OpChType = "wAFROC", lesDistr = lesDistr, legendPosition = "bottom", nlfRange = c(0, 1), llfRange = c(0, 1)), ret, check.environment = FALSE)
#   t <- PlotRsmOpChr(mu = mu, lambda = lambda, nu = nu,  zeta1 = zeta1, OpChType = "wAFROC", lesDistr = lesDistr, legendPosition = "bottom", nlfRange = c(0, 1), llfRange = c(0, 1))
#   # expect_is(t$wAFROCPlot, "ggplot")
# 
# })
