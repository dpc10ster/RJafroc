...
test_that("Compare current to original code: DBMH", {
  ...
  newFStatsRRRC <- newValues$RRRC$FTests
  ...
  newFStatsFRRC <- newValues$FRRC$FTests
  ...
  newFStatsRRFC <- newValues$RRFC$FTests
  ...
  newciDiffTrtRRRC <- as.vector(unlist(newValues$RRRC$ciDiffTrt[,-1]))
  ...
  newciAvgRdrEachTrtFRRC <- as.vector(unlist(newValues$FRRC$ciAvgRdrEachTrt[,-1]))
  ...
  newciDiffTrtRRFC <- as.vector(unlist(newValues$RRFC$ciDiffTrt[,-1]))
  ...
  newciAvgRdrEachTrtRRFC <- as.vector(unlist(newValues$RRFC$ciAvgRdrEachTrt[,-1]))
...
test_that("Compare current to original code: ORH", {
  ...
  newciAvgRdrEachTrtRRRC <- as.vector(unlist(newValues$RRRC$ciAvgRdrEachTrt[,-1]))
  for (i in 1: length(orgciAvgRdrEachTrtRRRC)){
    x <- orgciAvgRdrEachTrtRRRC[i]
    y <- newciAvgRdrEachTrtRRRC[i]
    expect_equal(x, y, tolerance = 0.00001, scale = abs(x))
  ...
  newciDiffTrtFRRC <- as.vector(unlist(newValues$FRRC$ciDiffTrt[,-1]))
  for (i in 1: length(orgciDiffTrtFRRC)){
    x <- orgciDiffTrtFRRC[i]
    y <- newciDiffTrtFRRC[i]
    expect_equal(x, y, tolerance = 0.00001, scale = abs(x))
