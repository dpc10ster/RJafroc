...
EstimateVarCovCrossed <- function(NL, LL, perCase, IDs, weights, maxNL, maxLL, FOM, avgIndx) {
  ...
  jkFOMArray1 <- apply(jkFOMArray, c(2, 3, 4), mean)
  fomArray1 <- apply(jkFOMArray1, c(1, 2), mean)
  jkFOMArray2 <- apply(jkFOMArray, c(1, 3, 4), mean)
  fomArray2 <- apply(jkFOMArray2, c(1, 2), mean)
  
  
  # if (avgIndx == 1){
  #   jkFOMArray1 <- apply(jkFOMArray, c(2, 3, 4), mean)
  #   fomArray1 <- apply(jkFOMArray, c(1, 2), mean)
  # }else{
  #   jkFOMArray2 <- apply(jkFOMArray, c(1, 3, 4), mean)
  #   fomArray2 <- apply(jkFOMArray, c(1, 2), mean)
  # }
