      ...
      for (b in 1:nBoots) {
        ...
        for (i in 1:I) {
          for (j in 1:J) {
            ...
            fomBsArray[i, j, b] <- MyFOM(nlBs, llBs, lesionVector, lesionID, lesionWeight, maxNL, fom)
          }
        }
      }
    } else if (fom %in% c("MaxLLF", "HrSe")) {
      ...
      for (b in 1:nBoots) {
        ...
        for (i in 1:I) {
          for (j in 1:J) {
            ...
            dim(nlBs) <- c(K, maxNL)
            dim(llBs) <- c(K2, max(lesionVector))
            fomBsArray[i, j, b] <- MyFOM(nlBs, llBs, lesionVector[kBs], lesionID[kBs, ], lesionWeight[kBs, ], maxNL, fom)
          }
        }
      }
    } else {
      ...
      for (b in 1:nBoots) {
        ...
        for (i in 1:I) {
          for (j in 1:J) {
            ...
            dim(nlBs) <- c(K, maxNL)
            dim(llBs) <- c(K2, max(lesionVector))
            fomBsArray[i, j, b] <- MyFOM(nlBs, llBs, lesionVector[kBs2], lesionID[kBs2, ], lesionWeight[kBs2, ], maxNL, fom)
          }
        }
      }
    }
