...
* `ModalityID`: a comma-separated listing of modalities (two or more modalities), each represented by a **unique integer**, that are applied to each case. In the example each cell has the value `0, 1`. **Each cell has to be text formatted.**
...
* A single reader dataset is allowed.
* A single reader treatment is **not** allowed.
