---
title: "Quick Start: Running DBM analysis"
author: "Dev P. Chakraborty"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Quick Start: Running DBM analysis}
...
## Introduction
This vignette is intended for those seeking a quick transiton from Windows *JAFROC* to *RJafroc*.
It is assumed that the user is familiar with the *JAFROC* data format and can analyze a dataset using the
Windows program. First, let me describe the structure in `R` of an `RJafroc` dataset. Later I will tell you
how to read a *JAFROC*  format file to create an `RJafroc` dataset.

## An ROC dataset
Let us start with a predefined dataset `{dataset3}` corresponding to the Franken ROC data.
Let us examine the structure of this dataset.

```{r}
str(dataset03)
```             

It shows a list with 8 members. The false positive ratings are contained in `{NL}`, an array
