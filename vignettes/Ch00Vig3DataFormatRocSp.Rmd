...
* The `ReaderID` column is a listing of readers each represented by a **unique string**. Note that, unlike the crossed design, the `ReaderID` column has  *single values*. **Each cell has to be text formatted - see Chapter 00 Vignette 1.**
...
* `ModalityID`: a comma-separated listing of modalities, each represented by a **unique string**. In the example shown below each cell has the value `1, 2`. **Each cell has to be text formatted - see Chapter 00 Vignette 1.**
