...
* The plotting function `PlotRsmOperatingCharacteristics()` returns a number of other objects, most importantly the plot, but here we use only the AUC. Additionally, it calls for the **intrinsic** RSM parameters, which is why we had to convert the **physical** to the **intrinsic** values. 
...
The plot is very close to linear. This makes it easy to design an interpolation function. In the following code block the first line fits `eswAfroc` vs. `esRoc` using the linear model `lm()` function constrained to pass through the origin (the minus one): `slope <- lm(eswAfroc ~ -1 + esRoc)`. One expects this constraint since for `deltaMu = 0` the effect size must be zero no matter how it is measured. 
...
The slope of the straight line fit is `slope`, where slope = `r slope$coefficients[1]` and `R2` = `r summary(slope)$r.squared`. Therefore, the conversion from ROC to wAFROC effect size is: `effectSizewAFROC = slope . effectSizeROC`. **The wAFROC effect size is twice the ROC effect size.** All that remains is to calculate the variance componenents using the two FOMs:
