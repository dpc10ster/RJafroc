...
* The list members `lesionVector`, `lesionID` and `lesionWeight` are all arrays with dimensions
    `[1:67,1]` containing ones. Again, these imply 67 diseased cases.
* The fields `lesionVector`, `lesionID` and `lesionWeight`, while not needed for ROC data, 
...
```{r}
ds <- DfReadDataFile(sysFileName, newExcelFileFormat = FALSE) ##!!!DPC!!!
``` 
, 
...
* `ret$FTestStatsRRRC$ddfRRRC` is the denominator degrees of freedom of the 
F-statistic.
...
* `ret$FTestStatsFRRC$fFRRC` is the F-statistic for fixed-reader
...
* `ret$FTestStatsFRRC$ndfFRRC` is the numerator degrees of freedom of the F-statistic,
...
* `ret$FTestStatsFRRC$ddfFRRC` is the denominator degreesof freedom of the F-statistic,
...
* `ret$ciAvgRdrEachTrtFRRC` is the 95% CI of reader-average
...
* `ret$msAnovaEachRdrFRRC` is the mean-squares ANOVA
for each reader
...
* `ret$FTestStatsRRFC$fRRFC` is the F-statistic for
